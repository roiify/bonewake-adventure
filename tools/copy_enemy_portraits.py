#!/usr/bin/env python3
"""Copy each enemy's south.png from V1 Enemies into the game as
<slug>_portrait.png. Used by the pre-battle preview thumbnails
(south = facing camera, cleaner portrait look)."""
import shutil
...
for slug, prefix in MAP.items():
    folders = list(SRC.glob(f"{prefix}*"))
    if not folders:
        fail.append((slug, "no folder"))
        continue
    candidates = list(folders[0].rglob("south.png"))
    if not candidates:
        fail.append((slug, "no south.png"))
        continue
    shutil.copy(candidates[0], DEST / f"{slug}_portrait.png")
    ok.append(slug)
print(f"copied portraits: {len(ok)}/{len(MAP)}")
